import os
import subprocess
import json
import tempfile
from PIL import Image
import numpy as np

# === CONFIGURATION ===
FPS_STANDARD = 1     # Experimental: 1 frame every second
FPS_CIRCLE = 1       # 1 frame every second
STRIP_HEIGHT = 100   # Height of 1px-wide strips
PROCESSED_FILE = "processed_files_experimental.json"
FRAME_ROOT = "frames"
CIRCLE_ROOT = "circle_data"
EXPERIMENTAL_SUFFIX = "_experimental"
IMAGE_SUFFIXES = (".jpg", ".jpeg", ".png")


def source_matches_last_video(processed, video_path, folder_name):
    last_video = processed.get("last_video", {})
    stored_path = last_video.get("path")
    return bool(stored_path) and (
        os.path.realpath(os.path.expanduser(stored_path))
        == os.path.realpath(os.path.expanduser(video_path))
        and last_video.get("folder") == folder_name
    )


def valid_folder_name(folder_name):
    return bool(folder_name) and folder_name not in {".", ".."} and os.path.basename(folder_name) == folder_name


def numbered_output_paths(output_dir, prefix, suffixes):
    if not os.path.isdir(output_dir):
        return []
    matches = []
    for file in os.listdir(output_dir):
        stem = file[len(prefix) : file.rfind(".")]
        if file.startswith(prefix) and file.lower().endswith(suffixes) and stem.isdigit():
            matches.append((int(stem), os.path.join(output_dir, file)))
    return [path for _, path in sorted(matches)]


def has_contiguous_numbered_outputs(output_dir, prefix, suffixes):
    paths = numbered_output_paths(output_dir, prefix, suffixes)
    numbers = [int(os.path.basename(path)[len(prefix) : os.path.basename(path).rfind(".")]) for path in paths]
    return bool(numbers) and numbers == list(range(1, len(numbers) + 1))


def extract_to_new_directory(output_dir, prefix, suffixes, build_command):
    parent = os.path.dirname(output_dir) or "."
    os.makedirs(parent, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix=f".{os.path.basename(output_dir)}-", dir=parent) as staging_dir:
        subprocess.run(build_command(staging_dir), check=True)
        if not has_contiguous_numbered_outputs(staging_dir, prefix, suffixes):
            raise RuntimeError("Extraction did not produce a contiguous numbered output sequence.")
        if os.path.isdir(output_dir):
            if os.listdir(output_dir):
                raise RuntimeError(f"Refusing to replace non-empty output directory: {output_dir}")
            os.rmdir(output_dir)
        os.replace(staging_dir, output_dir)

# === FRAME EXTRACTION (STANDARD MODE) ===
def extract_frames(video_path, output_dir, fps):
    """Extract full frames using ffmpeg with HDR tone mapping."""
    def build_command(staging_dir):
        return [
            "ffmpeg", "-an", "-sn",
            "-i", video_path,
            "-map", "0:v",
            "-vf", (
                f"fps={fps},"
                "zscale=t=linear:npl=100,"
                "format=gbrpf32le,"
                "zscale=p=bt709,"
                "tonemap=hable,"
                "zscale=t=bt709,"
                "format=yuv420p"
            ),
            "-q:v", "1", "-fps_mode", "vfr",
            "-loglevel", "warning", "-hide_banner", "-stats",
            os.path.join(staging_dir, "frame_%04d.jpg")
        ]

    cmd = build_command(output_dir)
    print(f"[>] Extracting frames (standard): {' '.join(cmd)}")
    extract_to_new_directory(output_dir, "frame_", IMAGE_SUFFIXES, build_command)
    print("[✓] Frame extraction complete.")

# === METADATA (Standard mode) ===
def calculate_frame_data(image_path):
    with Image.open(image_path) as source:
        np_img = np.array(source.convert("RGB"), dtype=np.float32)
    avg_color = np_img.mean(axis=(0, 1))
    brightness = (0.299 * avg_color[0]) + (0.587 * avg_color[1]) + (0.114 * avg_color[2])
    max_c, min_c = np.max(avg_color), np.min(avg_color)
    saturation = (max_c - min_c) / (max_c + 1e-5)
    return {
        "frame": os.path.basename(image_path),
        "color": [int(avg_color[0]), int(avg_color[1]), int(avg_color[2])],
        "brightness": float(brightness),
        "saturation": float(saturation)
    }

def save_metadata(metadata, frame_dir):
    output_file = os.path.join(frame_dir, "data.json")
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(metadata, f, indent=2)
    print(f"[✓] Metadata saved to {output_file}")

# === CIRCLE MODE EXTRACTION (Direct HDR Tone-Mapped Strips) ===
def extract_circle_strips(video_path, output_dir, fps=1, strip_height=100):
    """Extract 1px-wide tone-mapped strips directly using ffmpeg."""
    def build_command(staging_dir):
        return [
            "ffmpeg", "-an", "-sn",
            "-i", video_path,
            "-map", "0:v",
            "-vf", (
                f"fps={fps},"
                "zscale=t=linear:npl=100,"
                "format=gbrpf32le,"
                "zscale=p=bt709,"
                "tonemap=hable,"
                "zscale=t=bt709,"
                "format=yuv420p,"
                f"scale=1:{strip_height}"
            ),
            "-q:v", "1", "-fps_mode", "vfr",
            "-loglevel", "warning", "-hide_banner", "-stats",
            os.path.join(staging_dir, "strip_%04d.png")
        ]

    cmd = build_command(output_dir)
    print(f"[>] Extracting 1px strips (circle mode): {' '.join(cmd)}")
    extract_to_new_directory(output_dir, "strip_", (".png",), build_command)
    print("[✓] 1px strips extraction complete.")

# === TRACKING PROCESSED FILES ===
def load_processed():
    if os.path.exists(PROCESSED_FILE):
        with open(PROCESSED_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    return {}

def save_processed(data):
    with open(PROCESSED_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2)

# === MAIN ===
def main():
    processed = load_processed()
    reuse = "n"
    video_path = ""
    folder_name = ""

    # Reuse last video
    if "last_video" in processed:
        video_path = processed["last_video"]["path"]
        folder_name = processed["last_video"]["folder"]
        reuse = input(f"Reuse last video? ({video_path}) [y/n]: ").strip().lower()

    if reuse != "y":
        video_path = input("Enter full path to video file: ").strip()
        folder_name = input("Enter folder name (e.g. 'Aliens (1986) - tt0090605'): ").strip()

    if not os.path.isfile(video_path):
        print("[✗] Video not found.")
        return
    if not valid_folder_name(folder_name):
        print("[✗] Folder name must be a single non-empty directory name.")
        return

    # Experimental runs always write to suffixed folders to keep originals untouched.
    if not folder_name.endswith(EXPERIMENTAL_SUFFIX):
        folder_name = f"{folder_name}{EXPERIMENTAL_SUFFIX}"

    # Choose mode
    mode = input("Choose mode: [1] Standard (radial/vertical) [2] Circle (donut poster): ").strip()
    mode = "2" if mode == "2" else "1"

    if mode == "1":
        frame_dir = os.path.join(FRAME_ROOT, folder_name)
        os.makedirs(frame_dir, exist_ok=True)
        frame_paths = numbered_output_paths(frame_dir, "frame_", IMAGE_SUFFIXES)
        complete_frames = has_contiguous_numbered_outputs(frame_dir, "frame_", IMAGE_SUFFIXES)
        same_source = source_matches_last_video(processed, video_path, folder_name)
        if same_source and complete_frames:
            print("[!] Video previously processed – skipping extraction.")
        elif frame_paths:
            print("[✗] Existing frames are incomplete or do not match the selected source. Use a new folder or remove the stale generated frames.")
            return
        else:
            extract_frames(video_path, frame_dir, FPS_STANDARD)
        print("[>] Processing metadata...")
        metadata = []
        for i, file in enumerate(sorted(os.listdir(frame_dir)), 1):
            if file.lower().endswith(IMAGE_SUFFIXES):
                data = calculate_frame_data(os.path.join(frame_dir, file))
                metadata.append(data)
                if i % 100 == 0:
                    print(f"  Processed {i} frames...")
        save_metadata(metadata, frame_dir)

    else:
        circle_dir = os.path.join(CIRCLE_ROOT, folder_name)
        os.makedirs(circle_dir, exist_ok=True)
        strip_paths = numbered_output_paths(circle_dir, "strip_", (".png",))
        complete_strips = has_contiguous_numbered_outputs(circle_dir, "strip_", (".png",))
        same_source = source_matches_last_video(processed, video_path, folder_name)
        if same_source and complete_strips:
            print("[!] Circle data already processed – skipping extraction.")
        elif strip_paths:
            print("[✗] Existing circle strips are incomplete or do not match the selected source. Use a new folder or remove the stale generated strips.")
            return
        else:
            extract_circle_strips(video_path, circle_dir, FPS_CIRCLE, STRIP_HEIGHT)

    # Save last video info
    processed["last_video"] = {"path": video_path, "folder": folder_name}
    save_processed(processed)
    print("[✓] Processing complete.")

if __name__ == "__main__":
    main()
